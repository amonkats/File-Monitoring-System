...
<div id="nav">
	<ul>
		<li><a href="send_ass_comm.php">Create File</a></li>
		<li><a href="inbox_ass_comm.php">Inbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM ass_comm_edpr WHERE status ='Pending'";
...
?>
		...
		<li><a href="outbox_ass_comm.php">Outbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM outbox_ass_comm";
			$qry_data = mysqli_query($dbConnect, $qry_appr);
			$approve_count = mysqli_fetch_array($qry_data);
			$toatalCount = array_shift($approve_count);
			echo $toatalCount;
