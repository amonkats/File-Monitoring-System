...
<div id="nav">
	<ul>
		<li><a href="send_ge_mpa.php">Create File</a></li>
		<li><a href="inbox_ge_mpa.php">Inbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM ge_mpa_edpr WHERE status ='Pending'";
...
?>
		...
		<li><a href="outbox_ge_mpa.php">Outbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM outbox_ge_mpa";
			$qry_data = mysqli_query($dbConnect, $qry_appr);
			$approve_count = mysqli_fetch_array($qry_data);
			$toatalCount = array_shift($approve_count);
			echo $toatalCount;
