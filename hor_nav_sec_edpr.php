...
<div id="nav">
	<ul>
		<li><a href="send_sec_edpr.php">Create File</a></li>
		<li><a href="inbox_sec_edpr.php">Inbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM sec_edpr WHERE status ='Pending'";
...
?>
		...
		<li><a href="outbox_sec_edpr.php">Outbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM outbox_sec_edpr";
			$qry_data = mysqli_query($dbConnect, $qry_appr);
			$approve_count = mysqli_fetch_array($qry_data);
			$toatalCount = array_shift($approve_count);
			echo $toatalCount;
