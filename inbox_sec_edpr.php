 ...
 <img src="images/arms.jpg">
	...
	<?php 
	 ...
	 if (isset($_POST['btnsearch'])) {
	 		...
	 		$query = "SELECT * FROM `sec_edpr` WHERE CONCAT(`serialno`, `refno`, `file_origin`, `subject`, `sender`, `receiver`, `action`, `due_days`, `forward`, `date_sent`, `status`, `delete_file`) LIKE '%".$search_term."%'";
	 		$search_result = filterTable($query);
	 		$search_item = test_input($_POST['valueToSearch']);
	 	}

	 else{
	 		$query = "SELECT * FROM sec_edpr";
	 		$search_result = filterTable($query);
	 	}
	 ...
	 ?>
		<form name="search_box" action="inbox_sec_edpr.php" method="POST">
	 ...
	 <?php
		...
		<tr>
			...
			<?php
				...
				?>
				<td style="text-align: center;"><?php echo $row1[7];?> | <a href="view_countdown_sec_edpr.php?countdownId=<?php echo $row1[0];?>" style="color: blue; text-decoration: none;"><i>View</i></a>
			...
			<td><a href="activity_inbox_sec_edpr.php?searchId=<?php echo $row1[0];?>" style="color: blue; text-decoration: none;"><?php echo $row1[8];?></a></td>

			<td><?php echo $row1[9];?></td>

			<?php
			if ($row1[10] == 'Done') {?>
				<td><?php echo $row1[10];?> | <a href="update_assign_sec_edpr.php?updateId=<?php echo $row1[0];?>" style="color: blue; text-decoration: none;"><i>Update </i></td>

				<?php }?>
			<?php 
			if($row1[10] == 'Pending'){?>
				<td><b><label style="color: red"><?php echo $row1[10];?></label></b> | <a href="update_assign_sec_edpr.php?updateId=<?php echo $row1[0];?>" style="color: blue; text-decoration: none;"><i>Update</i></a></td>

				<?php }?>

				<td><a href="sec_edpr_deletefile.php?searchId=<?php echo $row1[0];?>" style="color: blue; text-decoration: none;"><?php echo $row1[11];?></a></td>
