	 ...
	 function test_input($keepData) {
	  ...
	  return $keepData;
	 	...
	 	if (isset($_POST['btnsearch'])) {
	 		...
	 		$query = "SELECT * FROM `outbox_se2_dpa` WHERE CONCAT(`serialno`, `refno`, `file_origin`, `subject`, `sender`, `receiver`, `action`, `due_days`, `date_sent`) LIKE '%".$search_term."%'";
	 		$search_result = filterTable($query);
	 		$search_item = test_input($_POST['valueToSearch']);
	 	}

	 else{
	 		$query = "SELECT * FROM outbox_se2_dpa";
	 		$search_result = filterTable($query);
	 	}
...
<section style="">
	...
	while($row1=mysqli_fetch_array($query)):;?>
		...
		<tr>
			...
			<td><a href="se2_dpa_del.php?searchId=<?php echo $row1[0];?>" style="color: blue; text-decoration: none;"><?php echo $row1[9];?></a></td>
