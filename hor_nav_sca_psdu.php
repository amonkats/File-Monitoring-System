...
<div id="nav">
	<ul>
		<li><a href="send_sca_psdu.php">Create File</a></li>
		<li><a href="inbox_sca_psdu.php">Inbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM sca_psdu_edpr WHERE status ='Pending'";
...
?>
		...
		<li><a href="outbox_sca_psdu.php">Outbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM outbox_sca_psdu";
			$qry_data = mysqli_query($dbConnect, $qry_appr);
			$approve_count = mysqli_fetch_array($qry_data);
			$toatalCount = array_shift($approve_count);
			echo $toatalCount;
