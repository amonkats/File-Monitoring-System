...
<div id="nav">
	<ul>
		<li><a href="send_e2_dpa.php">Create File</a></li>
		<li><a href="inbox_e2_dpa.php">Inbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM e2_dpa_edpr WHERE status ='Pending'";
...
?>
		...
		<li><a href="outbox_e2_dpa.php">Outbox (
			<?php
			$dbConnect = mysqli_connect('localhost', 'root', 'ak_94*jmv', 'edpr');
			$qry_appr = "SELECT COUNT(*) FROM outbox_e2_dpa";
			$qry_data = mysqli_query($dbConnect, $qry_appr);
			$approve_count = mysqli_fetch_array($qry_data);
			$toatalCount = array_shift($approve_count);
			echo $toatalCount;
